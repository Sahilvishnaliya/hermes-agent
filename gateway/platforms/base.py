...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    for denied in _media_delivery_denied_paths():
        ...
        # When $HOME is itself under a denied prefix (e.g. systemd StateDirectory=
        # puts HOME=/var/lib/hermes with /var/lib on the denylist), the home
        # is still the operator's own tree and should not be blocked.
        if home is not None and (_path_is_within(home, resolved_denied) or resolved_denied == home):
            continue
        return True
    return False
