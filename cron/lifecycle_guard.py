...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        # Return empty rather than the sentinel "hermes gateway restart": the
        # sentinel is itself a lifecycle command and would false-positive the
        # regex scan that runs on the concatenated prompt+script text. An
        # oversized file is not a lifecycle command; the empty return lets the
        # guard report "no lifecycle command found", which is correct.
        return ""
